...
export default function PreschoolActivities() {
  ...
  // Fetch activities - filter by center_id via students table
  const { data: activities = [], isLoading } = useQuery({
    queryKey: ["preschool-activities", user?.center_id, gradeFilter],
    queryFn: async () => {
      if (!user?.center_id) return [];
      // First fetch student IDs for this center
      ...
      let query = supabase
        ...
        .select("*, students(name, grade, center_id), activities(id, name, description), activity_types(name)")
